...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[ETF_EVT_ARG]]) : ETF_COMMAND_INTERFACE
		...
		do
			...
			if cmd_name ~ "set_number" then 
				 if attached {ETF_INT_ARG} args[1] as num and then TRUE and then attached {ETF_INT_ARG} args[2] as row and then TRUE and then attached {ETF_INT_ARG} args[3] as col and then TRUE then 
					 create {ETF_SET_NUMBER} Result.make ("set_number", [num.value , row.value , col.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "put_number" then 
				 if attached {ETF_INT_ARG} args[1] as num and then TRUE and then attached {ETF_INT_ARG} args[2] as row and then TRUE and then attached {ETF_INT_ARG} args[3] as col and then TRUE then 
					 create {ETF_PUT_NUMBER} Result.make ("put_number", [num.value , row.value , col.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "restart_game" then 
				 if TRUE then 
					 create {ETF_RESTART_GAME} Result.make ("restart_game", [], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			else 
	...
	do
		...
		loop
			...
			if cmd_name ~ "set_number" then 
				if NOT( ( args.count = 3 ) AND THEN attached {ETF_INT_ARG} args[1] as num and then TRUE and then attached {ETF_INT_ARG} args[2] as row and then TRUE and then attached {ETF_INT_ARG} args[3] as col and then TRUE) then 
